...
FETCH();
 ...
 function showLatest(data){
    data.forEach(detum =>{
        ...
        newPost.innerHTML =  `
            <div class="m-2 xl:max-w-sm bg-white border rounded-lg shadow-lg border-l-2  border-b-8 border-b-red-500 dark:bg-gray-800 dark:border-gray-700">
                <div class = "">
                    <img class="rounded-lg" src="${detum.cover_image}" alt="" />
                </div>
                <div class="p-5">
                    <p class="mb-3 font-normal text-gray-700 dark:text-gray-400"> ${detum.author.posted_date== undefined?'No publish date':detum.author.posted_date}</p>
                    <h5 class="mb-4 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">${detum.title}</h5>
